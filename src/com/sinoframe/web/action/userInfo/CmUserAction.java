...
@Results({
	...
	@Result(name="list",location="/system/userInfo/userList.jsp"),
...
public class CmUserAction extends BaseAction {
	...
	public String toadd() {
		 ...
		 this.sysSystemList = this.cmUserBS.findPageByQuery(hqls);
...
/*		// 查询密码历史表
		List<CmUserPasswordHistory> listTbUserPasswordHistory = cmUserPasswordHistoryBS
				.findPageByQuery(new SysPageInfo(0,3), 
					"from CmUserPasswordHistory where 1=1 and cmUser.userId='"+cmUser.getUserId()+"'", 
					null, new SysOrderByInfo("usedate","desc"));
			if(!listTbUserPasswordHistory.isEmpty()){
				for (int i = 0; i < listTbUserPasswordHistory.size(); i++) {
					if(Md5.getInstance().EncoderByMd5(pwd, "").equals(listTbUserPasswordHistory.get(i).getPassword())){					
						setSuccess(false);
						setMyError("请不要使用最近更改的三次密码");
						return "json";
					}
				}
				cmUser.setAccountPassword(Md5.getInstance().EncoderByMd5(pwd, ""));

...
@Results( {
		...
		@Result(name = "flightStrength", location = "/sms/training/dt/fjz/upgradeeligibility.jsp"),
...
public class UpgradeEligibilityAction extends BaseAction{
	...
	//EXCEL路径
	public String path = "E:\\e\\升级资格要求.xls";
	...
	public  String flightStrengthOut(){
		...
		System.out.println("");
			...
			for(int i=0 ;i<flightStrengthList.size();i++){
				...
				jxlUtil.writeExcel(Integer.valueOf(fs.getF1()),hang,6);
				jxlUtil.writeExcel(Integer.valueOf(fs.getF2()),hang,7);
				jxlUtil.writeExcel(Integer.valueOf(fs.getF3()),hang,8);
				jxlUtil.writeExcel(Integer.valueOf(fs.getF4()),hang,9);
				jxlUtil.writeExcel(Integer.valueOf(fs.getF5()),hang,10);
			...
			for(int x = beg ; x<ove ;x++){
				...
				if(buf.toString().split(",").length==8){ //下个EXCEL要替换 
					...
					for(int i=0;i<cc.length;i++){
						...
						f.setDw(cc[1]);
						f.setJx(cc[2]);
						f.setF1(Integer.valueOf(cc[3].trim()));
						f.setF2(Integer.valueOf(cc[4].trim()));
						f.setF3(Integer.valueOf(cc[5].trim()));
						f.setF4(Integer.valueOf(cc[6].trim()));
						f.setF5(Integer.valueOf(cc[7].trim()));

...
@Results( {
		...
		@Result(name = "flightStrength", location = "/sms/training/dt/fjz/retiredfor.jsp"),
...
/** 
 * 描述：2013年度退役情况 ACTION
 * @author LUJIE		
 */
public class RetiredInfoForAction extends BaseAction{
	...
	String strc = "";
	//SHEET 名称
	public String sheetName="2013月度退役情况";  //不同的页面名称 就修改下
	//EXCEL路径
	public String path = "E:\\e\\2013月度退役情况.xls";
	...
	public  String flightStrengthOut(){
		...
		if(flightStrength != null){
			 ...
			 for(int i=0;i<str.length;i++){
				 ...
				 flightStrengths.setY1(flightStrength.getY1().split(",")[i].trim());
				 
				 flightStrengths.setY2(flightStrength.getY2().split(",")[i].trim());
				 flightStrengths.setY3(flightStrength.getY3().split(",")[i].trim());
				 flightStrengths.setY4(flightStrength.getY4().split(",")[i].trim());
				 flightStrengths.setY5(flightStrength.getY5().split(",")[i].trim());
				 flightStrengths.setY6(flightStrength.getY6().split(",")[i].trim());
				 flightStrengths.setY7(flightStrength.getY7().split(",")[i].trim());
				 flightStrengths.setY8(flightStrength.getY8().split(",")[i].trim());
				 flightStrengths.setY9(flightStrength.getY9().split(",")[i].trim());
				 flightStrengths.setY10(flightStrength.getY10().split(",")[i].trim());
				 flightStrengths.setY11(flightStrength.getY11().split(",")[i].trim());
				 flightStrengths.setY12(flightStrength.getY12().split(",")[i].trim());
		...
		System.out.println("");
			...
			for(int i=0 ;i<flightStrengthList.size();i++){
				...
				jxlUtil.writeExcel(Integer.valueOf(fs.getY1()),hang,7);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY2()),hang,8);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY3()),hang,9);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY4()),hang,10);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY5()),hang,11);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY6()),hang,12);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY7()),hang,13);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY8()),hang,14);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY9()),hang,15);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY10()),hang,16);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY11()),hang,17);
				jxlUtil.writeExcel(Integer.valueOf(fs.getY12()),hang,18);
    ...
    public String flightStrength(){
		...
		try {
			...
			for(int x = beg ; x<ove ;x++){
				...
				if(buf.toString().split(",").length==16){ //下个EXCEL要替换 

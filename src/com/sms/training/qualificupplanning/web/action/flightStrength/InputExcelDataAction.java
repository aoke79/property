...
@Results( {
		...
		@Result(name = "TotlePlan", location = "/sms/training/dt/fjz/totlePlaneList2.jsp"),
 ...
 * @author LUJIE		
 */
public class InputExcelDataAction extends BaseAction{
	...
	String strc = "";
	//SHEET 名称
	public String sheetName="人机比";  //不同的页面名称 就修改下
	//EXCEL路径
	public String path = "E:\\e\\人机比.xls";
	...
	public  String flightStrengthOut(){
		...
		if(TotlePlan != null){
			 ...
			 for(int i=0;i<str.length;i++){
				...
				// TotlePlans.setZuoNuber((Integer.valueOf(this.ps2013.split(",")[i].trim());
				 TotlePlans.setPs2013s(this.ps2013.split(",")[i].trim());
				 TotlePlans.setPx2013s(this.px2013.split(",")[i].trim()); 
				 TotlePlans.setPs2014s(this.ps2014.split(",")[i].trim()); 
				 TotlePlans.setPx2014s(this.px2014.split(",")[i].trim()); 
				 TotlePlans.setPs2015s(this.ps2015.split(",")[i].trim()); 
				 TotlePlans.setPx2015s(this.px2015.split(",")[i].trim()); 
				 int c = 0;
	   ...
	   //}
			...
			for(int i=0 ;i<totlePlanList.size();i++){
				...
				jxlUtil.writeExcel(Float.valueOf(totlePlanList.get(i).getPs2013s()),hang,6);
				jxlUtil.writeExcel(Float.valueOf(totlePlanList.get(i).getPx2013s()),hang,7);
				jxlUtil.writeExcel(Float.valueOf(totlePlanList.get(i).getPs2014s()),hang,8);
				jxlUtil.writeExcel(Float.valueOf(totlePlanList.get(i).getPx2014s()),hang,9);
				jxlUtil.writeExcel(Float.valueOf(totlePlanList.get(i).getPs2015s()),hang,10);
				jxlUtil.writeExcel(Float.valueOf(totlePlanList.get(i).getPx2015s()),hang,11);
				//jxlUtil.writeExcel(Float.valueOf(totlePlanList.get(i).getRightCount()),hang,12);
			}
			if(jxlUtil != null){
				jxlUtil.closeJxlOut();
			}
			...
			for(int x = beg ; x<ove ;x++){
				...
				if(buf.toString().split(",").length==9){ //下个EXCEL要替换 12
					...
					for(int i=0;i<cc.length;i++){
						...
						f.setPs2013s(String.valueOf(cc[2+b]));
						f.setPx2013s(String.valueOf(cc[3+b]));
						f.setPs2014s(String.valueOf(cc[4+b]));
						f.setPx2014s(String.valueOf(cc[5+b]));
						f.setPs2015s(String.valueOf(cc[6+b]));
						f.setPx2015s(String.valueOf(cc[7+b]));
...
/*			for(TotlePlan f : totlePlanList){
			...
			// totlePlanList = nullInfo;
			 ...
			 this.message =  this.getFailMessage(this.path+" 该文件无法正常读取");

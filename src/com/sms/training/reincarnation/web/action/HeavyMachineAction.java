...
@Results({
	...
	@Result(name="list",location="sms/training/reincarnation/heavyMachine.jsp"),
...
public class HeavyMachineAction extends BaseAction {
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from HeavyMachine where 1=1  order by id "; //phase
			
			List<HeavyMachine> ss = this.cmUserBS.findPageByQuery(hqls);
			for(HeavyMachine o :  ss){
				
				for(PhaseInfo c : phaseInfoList){
					
					 if(o.getPhase().equals(c.getId())){
						 this.heavyMachineList.add(o);
						 continue;
					 }
				}
		   ...
		   //----
		    List<HeavyMachine> ous = new ArrayList<HeavyMachine>();
            String info = "";
		    for(HeavyMachine o : this.heavyMachineList){
				  ...
				  if(o.getType1id() != null && !"".equals(o.getType1id())){
					  Cmodel c = this.cmUserBS.findById(Cmodel.class, o.getType1id());
					  if(c != null){
						  o.setType1Name(c.getTypename());
					  }  else{
						  o.setType1Name(info);
					  } 
				  }
		    ...
		    //多条数据进行合并
			List<HeavyMachine> ouc = ous;
			
		    for(HeavyMachine o : ous){
			    for(HeavyMachine o1 : ouc){
			    	 if(o1.getType1id() != null && o.getType1id()!=null  && o1.getDeptid() != null && o.getPhase() != null && o.getDeptid() != null && o1.getPhase() != null ){
			    		  int i = ouc.indexOf(o1);
			    		   if( o1.getType1id().equals(o.getType1id()) &&  o1.getDeptid().equals(o.getDeptid()) && o.getTypeid().equals(o1.getTypeid())){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getPhase(),o1.getCross().toString()};
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<HeavyMachine> onew = new ArrayList<HeavyMachine>();
		    for(HeavyMachine os : ouc){
		    	if(s.indexOf(os.getDeptName()+os.getTypeName()+os.getType1Name()) == -1){
		    	...
		    	s += os.getDeptName()+os.getTypeName()+os.getType1Name()+",";
		    }
		    
		    
		    //----
		   List<HeavyMachine>  oc = new ArrayList<HeavyMachine>();
		   for( HeavyMachine  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<HeavyMachine> on = new ArrayList<HeavyMachine>();
		 ...
		 this.heavyMachineList = on;
		 
		 
		 //计算补加的行数
		 if(this.heavyMachineList != null){
			  this.ands = this.hang-this.heavyMachineList.size();
		 }
		 ...
		 List<HeavyMachine> us = new ArrayList<HeavyMachine>();
		 if(rangeSettingsList != null && rangeSettingsList.size()!=0){
		 for(RangeSettings r : rangeSettingsList){
			 for(HeavyMachine u : this.heavyMachineList){
	...
	public String doadd() {
		...
		try {
			...
			String hql = " delete HeavyMachine ";
			...
			for(int i=0;i<deptid.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	HeavyMachine upg = new HeavyMachine();
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setCross(Integer.valueOf(this.degree.split(",")[start+cc].trim()));
			    	}
			    	    //获取匹配标识 进行存储
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
				       if(upg.getDeptid() != null && upg.getTypeid() != null && upg.getDeptid().length() !=0 && upg.getTypeid().length() !=0 && upg.getType1id() != null && upg.getType1id().length() !=0){
				    	  
				    	   //过滤重复
				    	   String hqls = " from HeavyMachine where 1=1 and deptid ='"+upg.getDeptid()+"' and typeid='"+upg.getTypeid()+"' and type1id='"+upg.getType1id()+"' order by phase ";
				    	   List<HeavyMachine> hm = this.cmUserBS.findPageByQuery(hqls);
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "heavy-machine!toadd.do");

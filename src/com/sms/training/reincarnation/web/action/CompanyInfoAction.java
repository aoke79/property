...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/companyinfomde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/companyinfo.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/companyinfoadd.jsp"),
...
public class CompanyInfoAction extends BaseAction {
	...
	public String toadd() {
		try{
			 String hqls = "from CompanyInfo where 1=1 ";
			 this.companyInfoList = this.cmUserBS.findPageByQuery(hqls);
			getUserList();
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doadd() {
		...
		try {
			...
			if(this.companyInfo !=null ){
				this.cmUserBS.save(this.companyInfo);
			}
			this.message = this.getSuccessMessage(getText("addSuccess"), "tbuser", "forward", "company-info!toadd.do");
	...
	public String toupdate() {
		try{
                this.companyInfo = this.cmUserBS.findById(CompanyInfo.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.cmUserBS.update(this.companyInfo);
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "company-info!toadd.do?pageNum="
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(CompanyInfo.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.companyInfo=cmUserBS.findById(CompanyInfo.class,condition[i].trim());
	                   this.cmUserBS.delete(this.companyInfo);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "company-info!toadd.do");
	...
	private void getUserList() {
        ...
        String counthql="select count(*) from CompanyInfo where 1=1  ";

		if(query.get("like_deptname")!=null && !query.get("like_deptname").equals("")){
			query.put("like_deptname", query.get("like_deptname"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
...
this.companyInfoList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

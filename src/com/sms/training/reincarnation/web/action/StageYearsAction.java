...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/stageyearsmde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/stageyears.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/stageyearsadd.jsp"),
 ...
 * @author LUJIE Month
 */
public class StageYearsAction extends BaseAction {

	private static final long serialVersionUID = 3419081097679263763L;


	List<StageYears> stageYearsList = new ArrayList<StageYears>();
	

	private StageYears stageYears = new StageYears();
	...
	public StageYears getStageYears() {
		return stageYears;
	}
	public void setStageYears(StageYears stageYears) {
		this.stageYears = stageYears;
	}
	CmUser cmUser;
	SysMenu sysMenu;
	...
	public String toadd() {
		try{
			 String hqls = "from StageYears where 1=1 ";
	...
	public String doadd() {
		...
		try {
			...
			if(this.stageYears !=null ){
				this.cmUserBS.save(this.stageYears);
			}
			this.message = this.getSuccessMessage(getText("addSuccess"), "tbuser", "forward", "stage-years!toadd.do");
	...
	public String toupdate() {
		try{
                this.stageYears = this.cmUserBS.findById(StageYears.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.cmUserBS.update(this.stageYears);
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "stage-years!toadd.do?pageNum="
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(StageYears.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.stageYears=cmUserBS.findById(StageYears.class,condition[i].trim());
	                   this.cmUserBS.delete(this.stageYears);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "stage-years!toadd.do");
	...
	private void getUserList() {
		...
		//by lujie
        CmUser cmUser = (CmUser) ActionContext.getContext().getSession().get("user");	
        String 	hql="from StageYears where 1=1 ";
        String counthql="select count(*) from StageYears where 1=1  ";

		if(query.get("like_year")!=null && !query.get("like_year").equals("")){
			query.put("like_year", query.get("like_year"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());

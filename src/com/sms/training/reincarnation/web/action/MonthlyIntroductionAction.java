...
@Results({
	...
	@Result(name="list",location="/sms/training/reincarnation/monthlyIntroduction.jsp"),
...
/**
 * 描述：月度引进飞机计划 MO_UP_IN
 * @author LUJIE
 */
public class MonthlyIntroductionAction extends BaseAction {
	
	
	List<MonthlyIntroduction> monthlyIntroductionList = new ArrayList<MonthlyIntroduction>();
	...
	public List<MonthlyIntroduction> getMonthlyIntroductionList() {
		return monthlyIntroductionList;
	}
	public void setMonthlyIntroductionList(
			List<MonthlyIntroduction> monthlyIntroductionList) {
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from MonthlyIntroduction where 1=1  order by  id ";//degree
			
			List<MonthlyIntroduction> ss = this.cmUserBS.findPageByQuery(hqls);
			for(MonthlyIntroduction o :  ss){
		   ...
		   //----
		    List<MonthlyIntroduction> ous = new ArrayList<MonthlyIntroduction>();
            String info = "";
		    for(MonthlyIntroduction o : this.monthlyIntroductionList){
		    ...
		    //多条数据进行合并
			List<MonthlyIntroduction> ouc = ous;
			
		    for(MonthlyIntroduction o : ous){
			    for(MonthlyIntroduction o1 : ouc){
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<MonthlyIntroduction> onew = new ArrayList<MonthlyIntroduction>();
		    for(MonthlyIntroduction os : ouc){
		    ...
		    //----
		   List<MonthlyIntroduction>  oc = new ArrayList<MonthlyIntroduction>();
		   for( MonthlyIntroduction  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<MonthlyIntroduction> on = new ArrayList<MonthlyIntroduction>();
		 ...
		 List<MonthlyIntroduction> us = new ArrayList<MonthlyIntroduction>();
		 if(rangeSettingsList != null && rangeSettingsList.size()!=0){
		 for(RangeSettings r : rangeSettingsList){
			 for(MonthlyIntroduction u : this.monthlyIntroductionList){
	...
	public String doadd() {
		...
		try {
			...
			String hql = " delete MonthlyIntroduction ";
			...
			for(int i=0;i<typeid.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	MonthlyIntroduction upg = new MonthlyIntroduction();
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
			    	    if( upg.getTypeid() != null  && upg.getTypeid().length() !=0){
			    	    	...
			    	    	String hqls =" from MonthlyIntroduction where 1=1 and typeid='"+upg.getTypeid()+"' and deptid='"+upg.getDeptid()+"' order by id  ";
			    	    	List<MonthlyIntroduction> ueb = this.cmUserBS.findPageByQuery(hqls);
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "monthly-introduction!toadd.do");

...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/stagemonthsmde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/stagemonth.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/stagemonthadd.jsp"),
 ...
 * @author LUJIE month
 */
public class StageMonthAction extends BaseAction {


	private static final long serialVersionUID = -4201251554316057595L;


	List<StageMonth> stageMonthList = new ArrayList<StageMonth>();
	

	private StageMonth stageMonth = new StageMonth();
	


	public List<StageMonth> getStageMonthList() {
		return stageMonthList;
	}
	public void setStageMonthList(List<StageMonth> stageMonthList) {
		this.stageMonthList = stageMonthList;
	}
	public StageMonth getStageMonth() {
		return stageMonth;
	}
	public void setStageMonth(StageMonth stageMonth) {
	...
	public String toadd() {
		try{
			 String hqls = "from StageMonth where 1=1 ";
			 this.stageMonthList = this.cmUserBS.findPageByQuery(hqls);
			getUserList();
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doadd() {
		...
		try {
			...
			if(this.stageMonth !=null ){
				this.cmUserBS.save(this.stageMonth);
			}
			this.message = this.getSuccessMessage(getText("addSuccess"), "tbuser", "forward", "stage-month!toadd.do");
	...
	public String toupdate() {
		try{
                this.stageMonth = this.cmUserBS.findById(StageMonth.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.cmUserBS.update(this.stageMonth);
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "stage-month!toadd.do?pageNum="
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(StageMonth.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.stageMonth=cmUserBS.findById(StageMonth.class,condition[i].trim());
	                   this.cmUserBS.delete(this.stageMonth);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "stage-month!toadd.do");
	...
	private void getUserList() {
		...
		//by lujie
        CmUser cmUser = (CmUser) ActionContext.getContext().getSession().get("user");	
        String 	hql="from StageMonth where 1=1 ";
        String counthql="select count(*) from StageMonth where 1=1  ";

		if(query.get("like_month")!=null && !query.get("like_month").equals("")){
			query.put("like_month", query.get("like_month"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
...
this.stageMonthList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

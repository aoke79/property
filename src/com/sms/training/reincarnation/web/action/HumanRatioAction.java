...
@Results({
	...
	@Result(name="list",location="/sms/training/reincarnation/totlePlaneList2.jsp"),
 ...
 * @author LUJIE
 */
public class HumanRatioAction extends BaseAction {
    ...
    List<HumanRatio> humanRatioList = new ArrayList<HumanRatio>();
	...
	public List<HumanRatio> getHumanRatioList() {
		return humanRatioList;
	}
	public void setHumanRatioList(List<HumanRatio> humanRatioList) {
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from HumanRatio where 1=1  order by id  ";  //phase
			
			List<HumanRatio> ss = this.cmUserBS.findPageByQuery(hqls);
			for(HumanRatio o :  ss){
		   ...
		   //----
		    List<HumanRatio> ous = new ArrayList<HumanRatio>();
            String info = "";
		    for(HumanRatio o : this.humanRatioList){
		    ...
		    //多条数据进行合并
			List<HumanRatio> ouc = ous;
			
		    for(HumanRatio o : ous){
			    for(HumanRatio o1 : ouc){
			    	 if(o1.getTypeid() != null && o.getTypeid()!=null  && o1.getDeptid() != null && o.getPhase() != null && o.getDeptid() != null && o1.getPhase() != null ){
			    		  int i = ouc.indexOf(o1);
			    		   if( o1.getTypeid().equals(o.getTypeid()) &&  o1.getDeptid().equals(o.getDeptid()) && o.getTypeid().equals(o1.getTypeid())){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getPhase(),o1.getPlane().toString()};
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<HumanRatio> onew = new ArrayList<HumanRatio>();
		    for(HumanRatio os : ouc){
		    ...
		    //----
		   List<HumanRatio>  oc = new ArrayList<HumanRatio>();
		   for( HumanRatio  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<HumanRatio> on = new ArrayList<HumanRatio>();
	...
	public String doadd() {
		...
		try {
			...
			for(int i=0;i<deptid.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setPlane(Integer.valueOf(this.degree.split(",")[start+cc].trim()));
			    	}
			    	    //获取匹配标识 进行存储
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
				       if(upg.getDeptid() != null && upg.getTypeid() != null && upg.getDeptid().length() !=0 && upg.getTypeid().length() !=0 ){
				    	  //过滤重复的数据
				    	   List<HumanRatio> ur = this.cmUserBS.findPageByQuery(" from HumanRatio where 1=1 and deptid='"+upg.getDeptid()+"' and typeid='"+upg.getTypeid()+"' order by phase ");
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "human-ratio!toadd.do");

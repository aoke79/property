...
import com.sms.training.reincarnation.bean.HumanRatio;
import com.sms.training.reincarnation.bean.HumanRatiof;
import com.sms.training.reincarnation.bean.ModifiedProportion;
...
@Results({
	...
	@Result(name="list",location="/sms/training/reincarnation/totlePlaneList4.jsp"),
 ...
 * @author LUJIE
 */
public class ModifiedProportionAction extends BaseAction {
	
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -6929483798635531830L;



    List<HumanRatiof> humanRatioList = new ArrayList<HumanRatiof>();
	...
	List<ModifiedProportion>  modifiedProportionList = new ArrayList<ModifiedProportion>();
	
	List<PhaseInfo> phaseInfoList = new ArrayList<PhaseInfo>();
	

	public List<ModifiedProportion> getModifiedProportionList() {
		return modifiedProportionList;
	}
	public void setModifiedProportionList(
			List<ModifiedProportion> modifiedProportionList) {
		this.modifiedProportionList = modifiedProportionList;
	}
	...
	public List<HumanRatiof> getHumanRatioList() {
		return humanRatioList;
	}
	public void setHumanRatioList(List<HumanRatiof> humanRatioList) {
		this.humanRatioList = humanRatioList;
	}
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from ModifiedProportion where 1=1  order by id  ";  //phase
			
			List<ModifiedProportion> ss = this.cmUserBS.findPageByQuery(hqls);
			for(ModifiedProportion o :  ss){
				
				for(PhaseInfo c : phaseInfoList){
					
					 if(o.getPhase().equals(c.getId())){
						 this.modifiedProportionList.add(o);
						 continue;
					 }
				}
		   ...
		   //----
		    List<ModifiedProportion> ous = new ArrayList<ModifiedProportion>();
            String info = "";
		    for(ModifiedProportion o : this.modifiedProportionList){
...
/*				  if(o.getDeptid() != null && !"".equals(o.getDeptid())){
		    ...
		    //多条数据进行合并
			List<ModifiedProportion> ouc = ous;
			
		    for(ModifiedProportion o : ous){
			    for(ModifiedProportion o1 : ouc){
			    	// if(o1.getTypeid() != null && o.getTypeid()!=null  && o1.getDeptid() != null && o.getPhase() != null && o.getDeptid() != null && o1.getPhase() != null ){
			    		  int i = ouc.indexOf(o1);
			    		  // if( o1.getTypeid().equals(o.getTypeid()) &&  o1.getDeptid().equals(o.getDeptid()) && o.getTypeid().equals(o1.getTypeid())){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getPhase(),o1.getChange().toString()};
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<ModifiedProportion> onew = new ArrayList<ModifiedProportion>();
		    for(ModifiedProportion os : ouc){
		    	//if(s.indexOf(os.getDeptName()+os.getTypeName()+os.getTypeName()) == -1){
		    	...
		    	//s += os.getDeptName()+os.getTypeName()+os.getTypeName()+",";
		 //   }
		    
		    
		    //----
		   List<ModifiedProportion>  oc = new ArrayList<ModifiedProportion>();
		   for( ModifiedProportion  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<ModifiedProportion> on = new ArrayList<ModifiedProportion>();
		 ...
		 this.modifiedProportionList = on;
		 
		 
		 //计算补加的行数
		 if(this.modifiedProportionList != null){
			  this.ands = this.hang-this.modifiedProportionList.size();
		 }
		 ...
		 List<ModifiedProportion> us = new ArrayList<ModifiedProportion>();
		 if(rangeSettingsList != null && rangeSettingsList.size()!=0){
		 for(RangeSettings r : rangeSettingsList){
			 for(ModifiedProportion u : this.modifiedProportionList){
		 ...
		 List<ModifiedProportion> mp = new ArrayList<ModifiedProportion>();
			// this.modifiedProportionList  = us;
			 if(modifiedProportionList != null && modifiedProportionList.size() !=0){
				 mp.add(modifiedProportionList.get(0));
				 this.modifiedProportionList = mp;
			 }
         }catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doadd() {
		...
		try {
			...
			String hql = " delete ModifiedProportion ";
			this.cmUserBS.executeUpdate(hql);
			
			// 起始页
			int  y = 1;
			if(this.degree != null){
			//for(int i=0;i<degree.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	ModifiedProportion upg = new ModifiedProportion();
			    ...
			    //	upg.setDeptid(deptid);
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setChange(Integer.valueOf(this.degree.split(",")[start+cc].trim()));
			    	}
			    	    //获取匹配标识 进行存储
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
				     //  if(upg.getDeptid() != null && upg.getTypeid() != null && upg.getDeptid().length() !=0 && upg.getTypeid().length() !=0 ){
				    	  //过滤重复的数据
				    	 //  List<HumanRatio> ur = this.cmUserBS.findPageByQuery(" from HumanRatiof where 1=1 and deptid='"+upg.getDeptid()+"' and typeid='"+upg.getTypeid()+"' order by phase ");
				    	   //if(ur != null && phaseInfoList != null ){
				    		  // if(ur.size() != phaseInfoList.size() ){
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "modified-proportion!toadd.do");

...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/technicalGrademde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/technicalGrade.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/technicalGradeadd.jsp"),
 ...
 * @author LUJIE Month
 */
public class TechnicalGradeAction extends BaseAction {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6866094166318269113L;


	List<TechnicalGrade> stageYearsList = new ArrayList<TechnicalGrade>();
	

	public List<TechnicalGrade> getStageYearsList() {
		return stageYearsList;
	}
	public void setStageYearsList(List<TechnicalGrade> stageYearsList) {
		this.stageYearsList = stageYearsList;
	}
	public TechnicalGrade getTechnicalGrade() {
		return technicalGrade;
	}
	public void setTechnicalGrade(TechnicalGrade technicalGrade) {
		this.technicalGrade = technicalGrade;
	}
	private TechnicalGrade technicalGrade = new TechnicalGrade();
	...
	public String toadd() {
		try{
			 String hqls = "from TechnicalGrade where 1=1 ";
			 this.stageYearsList = this.cmUserBS.findPageByQuery(hqls);
			getUserList();
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doadd() {
		...
		try {
			...
			if(this.technicalGrade !=null ){
				this.cmUserBS.save(this.technicalGrade);
			}
			this.message = this.getSuccessMessage(getText("addSuccess"), "tbuser", "forward", "technical-grade!toadd.do");
	...
	public String toupdate() {
		try{
                this.technicalGrade = this.cmUserBS.findById(TechnicalGrade.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.cmUserBS.update(this.technicalGrade);
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "technical-grade!toadd.do?pageNum="
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(TechnicalGrade.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.technicalGrade=cmUserBS.findById(TechnicalGrade.class,condition[i].trim());
	                   this.cmUserBS.delete(this.technicalGrade);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "technical-grade!toadd.do");
	...
	private void getUserList() {
		...
		//by lujie
        CmUser cmUser = (CmUser) ActionContext.getContext().getSession().get("user");	
        String 	hql="from TechnicalGrade where 1=1 ";
        String counthql="select count(*) from TechnicalGrade where 1=1  ";

		if(query.get("like_degreename")!=null && !query.get("like_degreename").equals("")){
			query.put("like_degreename", query.get("like_degreename"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
...
this.stageYearsList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

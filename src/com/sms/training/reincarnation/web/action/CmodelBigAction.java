...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/cmodelBigmde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/cmodelBig.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/cmodelBigadd.jsp"),
 ...
 * @author LUJIE
 */
public class CmodelBigAction extends BaseAction {
	

	private static final long serialVersionUID = 3021101134215517552L;
	
	List<CmodelBig> cmodelList = new ArrayList<CmodelBig>();
	
	private CmodelBig cmodel = new CmodelBig();
	

	public List<CmodelBig> getCmodelList() {
		return cmodelList;
	}
	public void setCmodelList(List<CmodelBig> cmodelList) {
		this.cmodelList = cmodelList;
	}
	public CmodelBig getCmodel() {
		return cmodel;
	}
	public void setCmodel(CmodelBig cmodel) {
		this.cmodel = cmodel;
	}
	CmUser cmUser;
	SysMenu sysMenu;
	...
	public String toadd() {
		try{
			 String hqls = "from CmodelBig where 1=1 ";
	...
	public String doadd() {
		...
		try {
			...
			this.message = this.getSuccessMessage(getText("addSuccess"), "tbuser", "forward", "cmodel-big!toadd.do");
	...
	public String toupdate() {
		try{
                this.cmodel = this.cmUserBS.findById(CmodelBig.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "cmodel-big!toadd.do?pageNum="
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(CmodelBig.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.cmodel=cmUserBS.findById(CmodelBig.class,condition[i].trim());
	                   this.cmUserBS.delete(this.cmodel);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "cmodel-big!toadd.do");
	...
	private void getUserList() {
		...
		//by lujie
        CmUser cmUser = (CmUser) ActionContext.getContext().getSession().get("user");	
        String 	hql="from CmodelBig where 1=1 ";
        String counthql="select count(*) from CmodelBig where 1=1  ";

...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/fleetInfosmde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/fleetInfos.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/fleetInfosadd.jsp"),
 ...
 * @author LUJIE month
 */
public class FleetInfosAction extends BaseAction {


	private static final long serialVersionUID = -4201251554316057595L;


	List<FleetInfos> fleetInfosList = new ArrayList<FleetInfos>();
	

	private FleetInfos fleetInfos = new FleetInfos();
	

	public List<FleetInfos> getFleetInfosList() {
		return fleetInfosList;
	}
	public void setFleetInfosList(List<FleetInfos> fleetInfosList) {
		this.fleetInfosList = fleetInfosList;
	}
	public FleetInfos getFleetInfos() {
		return fleetInfos;
	}
	public void setFleetInfos(FleetInfos fleetInfos) {
	...
	public String toadd() {
		try{
			 List<FleetInfos> r2 = new ArrayList<FleetInfos>();
			 String hqls = "from FleetInfos where 1=1 ";
			 this.fleetInfosList = this.cmUserBS.findPageByQuery(hqls);
			getUserList();
			
			for( FleetInfos r : fleetInfosList){
				 if(r.getDeptid() != null && !r.getDeptid().equals("")){
					 CompanyInfo c = this.cmUserBS.findById(CompanyInfo.class,r.getDeptid());
					 if(c != null)  r.setDeptname(c.getDeptname());
				 }
				 if(r.getTypeid() != null && !r.getTypeid().equals("")){
					 Cmodel c = this.cmUserBS.findById(Cmodel.class,r.getTypeid());
					 if(c != null) r.setTypename(c.getTypename());
				 }
				 r2.add(r);
			}
			
			 this.fleetInfosList = r2;
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	private List<Cmodel> cmodelList = new ArrayList<Cmodel>();
	
	public String add() {

		this.companyInfoList = this.cmUserBS.findPageByQuery(" from CompanyInfo where 1=1 ");
	...
	public String doadd() {
		...
		try {
			...
			if(this.fleetInfos !=null ){
				this.cmUserBS.save(this.fleetInfos);
			}
			this.message = this.getSuccessMessage(getText("addSuccess"), "tbuser", "forward", "fleet-infos!toadd.do");
		
		}catch (Exception e) {
			//设定失败消息
			this.message = this.getFailMessage(getText("addFail")+"  飞机数输入了非数字 或者 长度过长");
			e.printStackTrace();
		}
		return SUCCESS;
	}
	...
	public String toupdate() {
		try{
			this.companyInfoList = this.cmUserBS.findPageByQuery(" from CompanyInfo where 1=1 ");
			this.cmodelList = this.cmUserBS.findPageByQuery(" from Cmodel where 1=1 ");
                this.fleetInfos = this.cmUserBS.findById(FleetInfos.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.cmUserBS.update(this.fleetInfos);
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "fleet-infos!toadd.do?pageNum="
								+ this.getPageNum() + "&numPerPage="
								+ this.getNumPerPage() + "&"
								+ Util.toStrQuery(query));
		}catch (Exception e) {
			this.message = this.getFailMessage(getText("updateFail") +" 飞机数输入了非数字 或者 长度过长");
			e.printStackTrace();
		}
		return SUCCESS;
	}
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(FleetInfos.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.fleetInfos=cmUserBS.findById(FleetInfos.class,condition[i].trim());
	                   this.cmUserBS.delete(this.fleetInfos);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "fleet-infos!toadd.do");
	...
	private void getUserList() {
        ...
        String 	hql="from FleetInfos where 1=1 ";
        String counthql="select count(*) from FleetInfos where 1=1  ";

		if(query.get("eq_plane")!=null && !query.get("eq_plane").equals("")){
			query.put("eq_plane", query.get("eq_plane"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
...
this.fleetInfosList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

...
import com.sms.training.reincarnation.bean.CaptainCompany;
...
@Results({
	...
	@Result(name="list",location="sms/training/reincarnation/captaincompany.jsp"),
 ...
 * @author LUJIE
 */
public class CaptainCompanyAction extends BaseAction {
	

	private static final long serialVersionUID = 3021101134215517552L;
	
	List<CaptainCompany> captainCompanyList = new ArrayList<CaptainCompany>();
	
	

	public List<CaptainCompany> getCaptainCompanyList() {
		return captainCompanyList;
	}
	public void setCaptainCompanyList(List<CaptainCompany> captainCompanyList) {
		this.captainCompanyList = captainCompanyList;
	}
	CmUser cmUser;
	SysMenu sysMenu;
	...
	public String toadd() {
		try{
			List<CaptainCompany> ca = new ArrayList<CaptainCompany>();
			
			
			
			 String hqls = "from CaptainCompany where 1=1 ";
			 this.captainCompanyList = this.cmUserBS.findPageByQuery(hqls);
			 //getUserList();
			
			
			for(CaptainCompany a : captainCompanyList){
				
				if(a.getDeptid() != null){
					CompanyInfo c = this.cmUserBS.findById(CompanyInfo.class,a.getDeptid());
				    if(c != null) a.setDeptNamet(c.getDeptname());
				}
				
				if(a.getXtypeid() != null){
					Cmodel c = this.cmUserBS.findById(Cmodel.class,a.getXtypeid());
				    if(c != null) a.setXdeptNamet(c.getTypename());
				}
				
				if(a.getTypeid() != null ){
					Cmodel c = this.cmUserBS.findById(Cmodel.class,a.getTypeid());
					if(c != null ) a.setTypeNamet(c.getTypename());
				}
				if(a.getYear() != null ){
					//StageYears s = this.cmUserBS.findById(StageYears.class,a.getYear());
					//if(s != null) a.setYearNamet(s.getYear());
				}
				if(a.getMonth() != null){
					//StageMonth s = this.cmUserBS.findById(StageMonth.class, a.getMonth());
					//if(s != null) a.setMonthNamet(s.getMonth());
				}
...
/*			if(validateIp()==false){
	...
	private void getUserList() {
		...
		//by lujie
        CmUser cmUser = (CmUser) ActionContext.getContext().getSession().get("user");	
        String 	hql="from CaptainCompany where 1=1 ";
        String counthql="select count(*) from CaptainCompany where 1=1  ";

		if(query.get("eq_numchange")!=null && !query.get("eq_numchange").equals("")){
			query.put("eq_numchange", query.get("eq_numchange"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
		...
		captainCompanyList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

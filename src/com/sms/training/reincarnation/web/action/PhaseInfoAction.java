...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/phaseInfomde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/phaseInfo.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/phaseInfoadd.jsp"),
 ...
 * @author LUJIE month
 */
public class PhaseInfoAction extends BaseAction {
	...
	public String toadd() {
		try{
			 String hqls = "from PhaseInfo where 1=1 ";
			 this.phaseInfoList = this.cmUserBS.findPageByQuery(hqls);
			getUserList();
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doadd() {
		...
		try {
			...
			if(this.phaseInfo !=null ){
				this.cmUserBS.save(this.phaseInfo);
			}
			this.message = this.getSuccessMessage(getText("addSuccess"), "tbuser", "forward", "phase-info!toadd.do");
	...
	public String toupdate() {
		try{
                this.phaseInfo = this.cmUserBS.findById(PhaseInfo.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.cmUserBS.update(this.phaseInfo);
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "phase-info!toadd.do?pageNum="
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(PhaseInfo.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.phaseInfo=cmUserBS.findById(PhaseInfo.class,condition[i].trim());
	                   this.cmUserBS.delete(this.phaseInfo);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "phase-info!toadd.do");
	...
	private void getUserList() {
		...
		//by lujie
        CmUser cmUser = (CmUser) ActionContext.getContext().getSession().get("user");	
        String 	hql="from PhaseInfo where 1=1 ";
        String counthql="select count(*) from PhaseInfo where 1=1  ";

		if(query.get("like_phase")!=null && !query.get("like_phase").equals("")){
			query.put("like_phase", query.get("like_phase"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
...
this.phaseInfoList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

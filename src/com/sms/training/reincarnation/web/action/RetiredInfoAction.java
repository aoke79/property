...
@Results({
	...
	@Result(name="list",location="/sms/training/reincarnation/retiredInfo.jsp"),
...
/**
 * 描述：退役情况表MO_UP_RETIRE
 * @author LUJIE
 */
public class RetiredInfoAction extends BaseAction {
	...
	public String toadd() {
		try{   
			//计算table的列数量
			String cb = " from StageYears where 1=1 order by id ";
			this.stageYearsList = this.cmUserBS.findPageByQuery(cb);
			if(this.stageYearsList != null)cs += this.stageYearsList.size();
			...
			//过滤 不存在的数据
			String hqls = "from RetiredInfo where 1=1  order by  id ";//degree
			
			List<RetiredInfo> ss = this.cmUserBS.findPageByQuery(hqls);
			for(RetiredInfo o :  ss){
				
				for(StageYears c : this.stageYearsList){
					
					 if(o.getYear() != null && o.getYear().equals(c.getId())){
						 this.retiredInfoList.add(o);
						 continue;
					 }
				}
		   ...
		   //----
		    List<RetiredInfo> ous = new ArrayList<RetiredInfo>();
            String info = "";
		    for(RetiredInfo o : this.retiredInfoList){
		    ...
		    //多条数据进行合并
			List<RetiredInfo> ouc = ous;
			
		    for(RetiredInfo o : ous){
			    for(RetiredInfo o1 : ouc){
			    	//o1.getTypeNameBig()!=null && o.getTypeNameBig()!=null && 
			    	 if(o1.getDeptid()!= null && o.getDeptid() != null &&o1.getTypeid() != null && o.getYear() != null && o.getTypeid() != null && o1.getYear() != null ){
			    		...
			    		//  o1.getTypeNameBig().equals(o.getTypeNameBig()) &&
			    		   if(o.getDeptid().equals(o1.getDeptid()) &&  o1.getTypeid().equals(o.getTypeid()) ){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getYear(),o1.getRetire().toString()}; //码表和值
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<RetiredInfo> onew = new ArrayList<RetiredInfo>();
		    for(RetiredInfo os : ouc){
		    	if(s.indexOf(os.getTypeid()+os.getDeptid()) == -1){
                    
		    		if(this.stageYearsList != null && this.stageYearsList.size() !=0){
		    			int i = this.stageYearsList.size()-os.getCompanyInfoList().size();
		    ...
		    //----
		   List<RetiredInfo>  oc = new ArrayList<RetiredInfo>();
		   for( RetiredInfo  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  for( RetiredInfo o :  oc ){
			  
			  Integer[] cbc = new Integer[this.stageYearsList.size()];
			  for(String[] sb : o.getCompanyInfoList()){
				 
				  for( StageYears c : this.stageYearsList) {
					  int i = this.stageYearsList.indexOf(c);
		  ...
		  List<RetiredInfo> on = new ArrayList<RetiredInfo>();
		  for(int i=0;i<oc.size();i++){
			  ...
			  for(int c =0 ; c<intArry.length;c++){
				  ...
				  if(intArry[c] != null && !intArry[c].toString().equals("null") && !intArry[c].toString().equals("")){
					  ...
					  String sname = this.stageYearsList.get(c).getId().toString();
		 ...
		 this.retiredInfoList = on;
		 
		 
		 //计算补加的行数
		 if(this.retiredInfoList != null){
			  this.ands = this.hang-this.retiredInfoList.size();
		 }
	...
	public String doadd() {
		...
		try {
			...
			for(int i=0;i<typeid.split(",").length;i++){
				...
				//行的长度
				int c = this.stageYearsList.size();
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	RetiredInfo upg = new RetiredInfo();
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setRetire(this.degree.split(",")[start+cc].trim());
			    	}
			    	    //获取匹配标识 进行存储
			    	    upg.setYear(this.stageYearsList.get(cc).getId());
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
			    	    if( upg.getTypeid() != null  && upg.getTypeid().length() !=0){
			    	    	...
			    	    	String hqls =" from RetiredInfo where 1=1 and typeid='"+upg.getTypeid()+"' and deptid='"+upg.getDeptid()+"' order by degree  ";
			    	    	List<RetiredInfo> ueb = this.cmUserBS.findPageByQuery(hqls);
			    	    	if(ueb != null && this.stageYearsList != null){
			    	    		if(ueb.size() != this.stageYearsList.size()){
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "retired-info!toadd.do");

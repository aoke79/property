...
@Results({
	...
	@Result(name="list",location="sms/training/reincarnation/upgradeEligibility.jsp"),
 ...
 * @author LUJIE
 */
public class UpgradeEligibilityAction extends BaseAction {
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from UpgradeEligibility where 1=1  order by  id";//degree
			
			List<UpgradeEligibility> ss = this.cmUserBS.findPageByQuery(hqls);
			for(UpgradeEligibility o :  ss){
				
				for(TechnicalGrade c : technicalGradeList){
					
					 if(o.getDegree().equals(c.getDegree())){
						 this.upgradeEligibilityList.add(o);
						 continue;
					 }
				}
		   ...
		   //----
		    List<UpgradeEligibility> ous = new ArrayList<UpgradeEligibility>();
            String info = "";
		    for(UpgradeEligibility o : this.upgradeEligibilityList){
				  ...
				  if(o.getTypeid() != null && !"".equals(o.getTypeid())){
					  ...
					  CmodelBig cbc = new CmodelBig();
					  if(c != null){
						  cbc= this.cmUserBS.findById(CmodelBig.class,c.getFsort());
					  }
		    ...
		    //多条数据进行合并
			List<UpgradeEligibility> ouc = ous;
			
		    for(UpgradeEligibility o : ous){
			    for(UpgradeEligibility o1 : ouc){
			    	//o1.getTypeNameBig()!=null && o.getTypeNameBig()!=null && 
			    	 if(o1.getTypeid() != null && o.getDegree() != null && o.getTypeid() != null && o1.getDegree() != null ){
			    		  int i = ouc.indexOf(o1);
			    		//  o1.getTypeNameBig().equals(o.getTypeNameBig()) &&
			    		   if( o1.getTypeid().equals(o.getTypeid()) && o.getTypeid().equals(o1.getTypeid())){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getDegree(),o1.getNeedtime().toString()}; //码表和值
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<UpgradeEligibility> onew = new ArrayList<UpgradeEligibility>();
		    for(UpgradeEligibility os : ouc){
		    	if(s.indexOf(os.getTypeName()) == -1){
		    ...
		    //----
		   List<UpgradeEligibility>  oc = new ArrayList<UpgradeEligibility>();
		   for( UpgradeEligibility  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<UpgradeEligibility> on = new ArrayList<UpgradeEligibility>();
		 ...
		 this.upgradeEligibilityList = on;
		 
		 
		 //计算补加的行数
		 if(this.upgradeEligibilityList != null){
			  this.ands = this.hang-this.upgradeEligibilityList.size();
		 }
		 ...
		 List<UpgradeEligibility> us = new ArrayList<UpgradeEligibility>();
		 if(rangeSettingsList != null && rangeSettingsList.size()!=0){
		 for(RangeSettings r : rangeSettingsList){
			 for(UpgradeEligibility u : this.upgradeEligibilityList){
				// CompanyInfo c = this.cmUserBS.findById(CompanyInfo.class,u.getId());
		...
		// this.upgradeEligibilityList  = us;
	...
	public String doadd() {
		...
		try {
			...
			String hql = " delete UpgradeEligibility ";
			...
			for(int i=0;i<typeid.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	UpgradeEligibility upg = new UpgradeEligibility();
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setNeedtime(this.degree.split(",")[start+cc].trim());
			    	}
			    	    //获取匹配标识 进行存储
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
			    	    if( upg.getTypeid() != null  && upg.getTypeid().length() !=0){
			    	    	...
			    	    	String hqls =" from UpgradeEligibility where 1=1 and typeid='"+upg.getTypeid()+"' order by degree  ";
			    	    	List<UpgradeEligibility> ueb = this.cmUserBS.findPageByQuery(hqls);
			    	    	if(ueb != null && this.technicalGradeList != null){
			    	    		if(ueb.size() != this.technicalGradeList.size()){
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "upgrade-eligibility!toadd.do");

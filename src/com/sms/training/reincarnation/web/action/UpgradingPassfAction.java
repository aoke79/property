...
import com.sms.training.reincarnation.bean.UpgradingPassf;
...
@Results({
	...
	@Result(name="list",location="sms/training/reincarnation/upgradingPass2.jsp"),
 ...
 * @author LUJIE
 */
public class UpgradingPassfAction extends BaseAction {
	
	
	List<UpgradingPassf> upgradingPassList = new ArrayList<UpgradingPassf>();
	...
	UpgradingPass up = new UpgradingPass();
	...
	public UpgradingPass getUp() {
		return up;
	}
	public void setUp(UpgradingPass up) {
		this.up = up;
	}
	public List<UpgradingPassf> getUpgradingPassList() {
		return upgradingPassList;
	}
	public void setUpgradingPassList(List<UpgradingPassf> upgradingPassList) {
	...
	public String toadd() {
		try{   
			//计算table的列数量
			String cb = " from TechnicalGrade where 1=1 order by degree ";
			this.technicalGradeList = this.cmUserBS.findPageByQuery(cb);
			if(technicalGradeList != null)cs += technicalGradeList.size();
			...
			//过滤 不存在的数据
			String hqls = "from UpgradingPassf where 1=1  order by id ";  //degree
			
			List<UpgradingPassf> ss = this.cmUserBS.findPageByQuery(hqls);
			for(UpgradingPassf o :  ss){
				
				for(TechnicalGrade c : technicalGradeList){
					
					 if(o.getDegree().equals(c.getDegree())){
						 this.upgradingPassList.add(o);
						 continue;
					 }
				}
		   ...
		   //----
		    List<UpgradingPassf> ous = new ArrayList<UpgradingPassf>();
            String info = "";
		    for(UpgradingPassf o : this.upgradingPassList){
				  ...
				  if(o.getDegree() != null && !"".equals(o.getDegree())){
					  TechnicalGrade tg = this.cmUserBS.findById(TechnicalGrade.class,o.getDegree());
					  if(tg != null){
						  o.setDegreeName(tg.getDegreename());
					  }else{
						  o.setDegreeName(info);
					  }
				  }
		    ...
		    //多条数据进行合并
			List<UpgradingPassf> ouc = ous;
			
		    for(UpgradingPassf o : ous){
			    for(UpgradingPassf o1 : ouc){
			    	 if(o1.getDeptid() != null && o.getDegree() != null && o.getDeptid() != null && o1.getDegree() != null ){
			    		  int i = ouc.indexOf(o1);
			    		   if(o1.getDeptid().equals(o.getDeptid()) && o.getTypeid().equals(o1.getTypeid())){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getDegree(),o1.getAd().toString()};
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<UpgradingPassf> onew = new ArrayList<UpgradingPassf>();
		    for(UpgradingPassf os : ouc){
		    	if(s.indexOf(os.getDeptName()+os.getTypeName()) == -1){
                    
		    		if(this.technicalGradeList != null && technicalGradeList.size() !=0){
		    			int i = technicalGradeList.size()-os.getCompanyInfoList().size();
		    	...
		    	s += os.getDeptName()+os.getTypeName()+",";
		    }
		    
		    
		    //----
		   List<UpgradingPassf>  oc = new ArrayList<UpgradingPassf>();
		   for( UpgradingPassf  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  for( UpgradingPassf o :  oc ){
			  
			  Integer[] cbc = new Integer[this.technicalGradeList.size()];
			  for(String[] sb : o.getCompanyInfoList()){
				 
				  for( TechnicalGrade c : technicalGradeList) {
					  int i = technicalGradeList.indexOf(c);
					    if(sb[0].equals(c.getDegree())){
					    	cbc[i] = i;
					    	break;
					    }else{
					    	//cbc[i]= 1000; 
		  ...
		  List<UpgradingPassf> on = new ArrayList<UpgradingPassf>();
		  for(int i=0;i<oc.size();i++){
			  ...
			  for(int c =0 ; c<intArry.length;c++){
				  ...
				  if(intArry[c] != null && !intArry[c].toString().equals("null") && !intArry[c].toString().equals("")){
					  ...
					  String sname = this.technicalGradeList.get(c).getDegree().toString();
		 ...
		 this.upgradingPassList = on;
		 
		 
		 //计算补加的行数
		 if(this.upgradingPassList != null){
			  this.ands = this.hang-this.upgradingPassList.size();
		 }
		 ...
		 List<UpgradingPassf> us = new ArrayList<UpgradingPassf>();
		 if(rangeSettingsList != null && rangeSettingsList.size()!=0){
		 for(RangeSettings r : rangeSettingsList){
			 for(UpgradingPassf u : upgradingPassList){
	...
	public String doadd() {
		...
		try {
			...
			String hql = " delete UpgradingPassf ";
			...
			for(int i=0;i<deptid.split(",").length;i++){
				...
				this.technicalGradeList = this.cmUserBS.findPageByQuery(cb);
				//行的长度
				int c = this.technicalGradeList.size();
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	UpgradingPassf upg = new UpgradingPassf();
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setAd(Integer.valueOf(this.degree.split(",")[start+cc].trim()));
			    	}
			    	    //获取匹配标识 进行存储
			    	    upg.setDegree(this.technicalGradeList.get(cc).getDegree());
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
				       if(upg.getDeptid() != null && upg.getTypeid() != null && upg.getDeptid().length() !=0 && upg.getTypeid().length() !=0){
				    	   ...
				    	   String hqls = " from UpgradingPassf where 1=1 and deptid='"+upg.getDeptid()+"' and typeid='"+upg.getTypeid()+"' order by degree ";
				    	   List<UpgradingPassf> ups = this.cmUserBS.findPageByQuery(hqls);
				    	   if(ups != null && this.technicalGradeList != null){
				    		   if(ups.size() != this.technicalGradeList.size()){
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "upgrading-passf!toadd.do");

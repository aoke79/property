...
@Results({
	...
	@Result(name="list",location="sms/training/reincarnation/upgradingPass.jsp"),
 ...
 * @author LUJIE
 */
public class UpgradingPassAction extends BaseAction {
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from UpgradingPass where 1=1  order by id ";  //degree
			
			List<UpgradingPass> ss = this.cmUserBS.findPageByQuery(hqls);
			for(UpgradingPass o :  ss){
		   ...
		   //----
		    List<UpgradingPass> ous = new ArrayList<UpgradingPass>();
            String info = "";
		    for(UpgradingPass o : this.upgradingPassList){
		    ...
		    //多条数据进行合并
			List<UpgradingPass> ouc = ous;
			
		    for(UpgradingPass o : ous){
			    for(UpgradingPass o1 : ouc){
			    	 if(o1.getDeptid() != null && o.getDegree() != null && o.getDeptid() != null && o1.getDegree() != null ){
			    		  int i = ouc.indexOf(o1);
			    		   if(o1.getDeptid().equals(o.getDeptid()) && o.getTypeid().equals(o1.getTypeid())){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getDegree(),o1.getPass().toString()};
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<UpgradingPass> onew = new ArrayList<UpgradingPass>();
		    for(UpgradingPass os : ouc){
		    ...
		    //----
		   List<UpgradingPass>  oc = new ArrayList<UpgradingPass>();
		   for( UpgradingPass  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<UpgradingPass> on = new ArrayList<UpgradingPass>();
	...
	public String doadd() {
		...
		try {
			...
			String hql = " delete UpgradingPass ";
			...
			for(int i=0;i<deptid.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	UpgradingPass upg = new UpgradingPass();
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setPass(Integer.valueOf(this.degree.split(",")[start+cc].trim()));
			    	}
			    	    //获取匹配标识 进行存储
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
				       if(upg.getDeptid() != null && upg.getTypeid() != null && upg.getDeptid().length() !=0 && upg.getTypeid().length() !=0){
				    	   ...
				    	   String hqls = " from UpgradingPass where 1=1 and deptid='"+upg.getDeptid()+"' and typeid='"+upg.getTypeid()+"' order by degree ";
				    	   List<UpgradingPass> ups = this.cmUserBS.findPageByQuery(hqls);
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "upgrading-pass!toadd.do");

...
@Results({
	...
	@Result(name="list",location="/sms/training/reincarnation/totlePlaneList3.jsp"),
 ...
 * @author LUJIE
 */
public class HumanRatiofAction extends BaseAction {
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from HumanRatiof where 1=1  order by id  ";  //phase
			
			List<HumanRatiof> ss = this.cmUserBS.findPageByQuery(hqls);
			for(HumanRatiof o :  ss){
				
				for(PhaseInfo c : phaseInfoList){
					
					 if(o.getPhase().equals(c.getId())){
						 this.humanRatioList.add(o);
						 continue;
					 }
				}
		   ...
		   //----
		    List<HumanRatiof> ous = new ArrayList<HumanRatiof>();
            String info = "";
		    for(HumanRatiof o : this.humanRatioList){
		    ...
		    //多条数据进行合并
			List<HumanRatiof> ouc = ous;
			
		    for(HumanRatiof o : ous){
			    for(HumanRatiof o1 : ouc){
			    	 if(o1.getTypeid() != null && o.getTypeid()!=null  && o1.getDeptid() != null && o.getPhase() != null && o.getDeptid() != null && o1.getPhase() != null ){
			    		  int i = ouc.indexOf(o1);
			    		   if( o1.getTypeid().equals(o.getTypeid()) &&  o1.getDeptid().equals(o.getDeptid()) && o.getTypeid().equals(o1.getTypeid())){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getPhase(),o1.getPolit().toString()};
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<HumanRatiof> onew = new ArrayList<HumanRatiof>();
		    for(HumanRatiof os : ouc){
		    ...
		    //----
		   List<HumanRatiof>  oc = new ArrayList<HumanRatiof>();
		   for( HumanRatiof  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<HumanRatiof> on = new ArrayList<HumanRatiof>();
		 ...
		 this.humanRatioList = on;
		 
		 
		 //计算补加的行数
		 if(this.humanRatioList != null){
			  this.ands = this.hang-this.humanRatioList.size();
		 }
		...
		try {
			...
			for(int i=0;i<deptid.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	HumanRatiof upg = new HumanRatiof();
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setPolit(Integer.valueOf(this.degree.split(",")[start+cc].trim()));
			    	}
			    	    //获取匹配标识 进行存储
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "human-ratiof!toadd.do");

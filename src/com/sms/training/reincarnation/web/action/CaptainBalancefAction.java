...
@Results({
	...
	@Result(name="list",location="sms/training/reincarnation/captainbalancef.jsp"),
 ...
 * @author LUJIE
 */
public class CaptainBalancefAction extends BaseAction {
	

	private static final long serialVersionUID = 3021101134215517552L;
	
	List<CaptainBalance> captainBalanceList = new ArrayList<CaptainBalance>();
	
	
	public List<CaptainBalance> getCaptainBalanceList() {
		return captainBalanceList;
	}
	public void setCaptainBalanceList(List<CaptainBalance> captainBalanceList) {
		this.captainBalanceList = captainBalanceList;
	}
	CmUser cmUser;
	SysMenu sysMenu;
	...
	public String toadd() {
		try{
			...
			List<CaptainBalance> cb = new ArrayList<CaptainBalance>();
			 String hqls = "from CaptainBalance where 1=1 ";
			 this.captainBalanceList = this.cmUserBS.findPageByQuery(hqls);
			getUserList();
			
			
			for(CaptainBalance ab : captainBalanceList){
				...
				if(ab.getNumelseoutneed() == null){
					ab.setNumelseoutneed(0);
				}
				
				if(ab.getNumpolitbeg() == null){
					ab.setNumpolitbeg(0);
				}
				if(ab.getNumelseoutaft() == null){
					ab.setNumelseoutaft(0);
				}
				
				if(ab.getNumpolitend() == null){
					ab.setNumpolitend(0);
				}
				ab.setQc(ab.getNumelseoutneed()-ab.getNumpolitbeg()+"");
				ab.setQm(ab.getNumelseoutaft()-ab.getNumpolitend()+"");
			 ...
			 List<CaptainBalance> us = new ArrayList<CaptainBalance>();
			 if(rangeSettingsList != null && rangeSettingsList.size()!=0){
			 for(RangeSettings r : rangeSettingsList){
				 for(CaptainBalance u : this.captainBalanceList){
					 CompanyInfo c = null;
					 Cmodel  cd = null;
					 if(u.getDeptid() != null){
...
/*			if(validateIp()==false){
	...
	private void getUserList() {
		...
		//by lujie
        CmUser cmUser = (CmUser) ActionContext.getContext().getSession().get("user");	
        String 	hql="from CaptainBalance where 1=1 ";
        String counthql="select count(*) from CaptainBalance where 1=1  ";

		if(query.get("eq_numplane")!=null && !query.get("eq_numplane").equals("")){
			query.put("eq_numplane", query.get("eq_numplane"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
		...
		captainBalanceList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

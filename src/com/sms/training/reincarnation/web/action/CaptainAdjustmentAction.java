...
import com.sms.training.reincarnation.bean.CaptainAdjustment;
...
@Results({
	...
	@Result(name="list",location="/sms/training/reincarnation/inputCompanyList.jsp"),
...
/**
 * 描述：公司间机长调整建议表 MO_UP_CHANGE_OUT
 * @author LUJIE
 */
public class CaptainAdjustmentAction extends BaseAction {

	private static final long serialVersionUID = -874612142976552630L;
	...
	private CaptainAdjustment  cam = new CaptainAdjustment();
	...
	public CaptainAdjustment getCam() {
		return cam;
	}
	public void setCam(CaptainAdjustment cam) {
		this.cam = cam;
	}
	...
	private List<CaptainAdjustment> captainAdjustmentList = new ArrayList<CaptainAdjustment>();
	
	public List<CaptainAdjustment> getCaptainAdjustmentList() {
		return captainAdjustmentList;
	}
	public void setCaptainAdjustmentList(
			List<CaptainAdjustment> captainAdjustmentList) {
		this.captainAdjustmentList = captainAdjustmentList;
	}
	public String toadd() {
		try{   
	
		String hqls = "  from CaptainAdjustment where 1=1 order by id ";
		this.captainAdjustmentList = this.cmUserBS.findPageByQuery(hqls);	
		List<CaptainAdjustment> cc = new ArrayList<CaptainAdjustment>();
		
		for(CaptainAdjustment cb : captainAdjustmentList){
			
			if(cb.getDeptinid() != null){
				CompanyInfo  com = this.cmUserBS.findById(CompanyInfo.class,cb.getDeptinid());
				if(com != null)cb.setDeptinName(com.getDeptname());
			}
			if(cb.getDeptoutid() != null){
				CompanyInfo	com = this.cmUserBS.findById(CompanyInfo.class,cb.getDeptoutid());
				if(com != null)cb.setDeptoutName(com.getDeptname());
			}
			if(cb.getTypeinid() != null){
				Cmodel cmo = this.cmUserBS.findById(Cmodel.class, cb.getTypeinid());
				if(cmo != null)cb.setTypeinName(cmo.getTypename());
			}
			if(cb.getTypeoutid() != null){
				Cmodel cmo = this.cmUserBS.findById(Cmodel.class, cb.getTypeoutid());
				if(cmo != null)cb.setTypeoutName(cmo.getTypename());
			}
			cc.add(cb);
		}
		this.captainAdjustmentList = cc;
		 ...
		 List<CaptainAdjustment> us = new ArrayList<CaptainAdjustment>();
		 if(rangeSettingsList != null && rangeSettingsList.size()!=0){
		 for(RangeSettings r : rangeSettingsList){
			 for(CaptainAdjustment u : this.captainAdjustmentList){
				 
				 CompanyInfo c = this.cmUserBS.findById(CompanyInfo.class,u.getDeptoutid());
				 Cmodel  cd = this.cmUserBS.findById(Cmodel.class,u.getTypeoutid());
				 
				 if(cd != null && c != null && cd.getTypename().equals(r.getTypeid())  && c.getDeptname().equals(r.getDeptid())){
					 us.add(u);
				 }
			 }
		 }
		//this.captainAdjustmentList  = us;
	...
	public String doadd() {
		...
		try {
			 ...
			 if( rangeSettingsList != null && rangeSettingsList.size() !=0){
				// for(RangeSettings r : rangeSettingsList){
					// CompanyInfo c = this.cmUserBS.findPageByQuery(hql)
					// select += "  and "+r.getDeptid()
				// }
			...
			if(this.degree != null){
				...
				for(int i=0;i<str.length;i++){
					CaptainAdjustment c = captainAdjustmentList.get(i);
					if(str[i] != null){
						c.setChange(str[i].trim());
					}else{
						c.setChange("0");
					}
				    info  = "第 " +(i+1) +" 行 输入了非数字 ";
					//info  = " 输入了非数字 ";
					this.cmUserBS.update(c);
				}
				
			}

			//forward upgrade-eligibility!toadd.do
			this.message = this.getSuccessMessage("保存成功! ", "tbuser", "", "");
		
		}catch (Exception e) {
			//设定失败消息
			this.message = this.getFailMessage("保存失败,"+info);
			e.printStackTrace();
		}
		return SUCCESS;
	}

...
@Results({
	...
	@Result(name="toupdate",location="sms/training/reincarnation/rangeSettingsmde.jsp"),
	@Result(location="/standard/ajaxDone.jsp"),
	
	@Result(name="list",location="sms/training/reincarnation/rangeSettings.jsp"),
	
	@Result(name="usersyslist",location="/system/userInfo/usersysbaoList.jsp"),
	
	@Result(name="add",location="sms/training/reincarnation/rangeSettingsadd.jsp"),
 ...
 * @author LUJIE month
 */
public class RangeSettingsAction extends BaseAction {


	private static final long serialVersionUID = -4201251554316057595L;


	List<RangeSettings> rangeSettingsList = new ArrayList<RangeSettings>();
	

	private RangeSettings rangeSettings = new RangeSettings();
	

	public List<RangeSettings> getRangeSettingsList() {
		return rangeSettingsList;
	}
	public void setRangeSettingsList(List<RangeSettings> rangeSettingsList) {
		this.rangeSettingsList = rangeSettingsList;
	}
	public RangeSettings getRangeSettings() {
		return rangeSettings;
	}
	public void setRangeSettings(RangeSettings rangeSettings) {
	...
	public String toadd() {
		try{
			 List<RangeSettings> r2 = new ArrayList<RangeSettings>();
			 String hqls = "from RangeSettings where 1=1 ";
			 this.rangeSettingsList = this.cmUserBS.findPageByQuery(hqls);
			 
			 for(RangeSettings r : rangeSettingsList){
				 String c = r.getDeptid()+"-"+r.getTypeid()+",";
				 this.dws +=c;
			 }
			 
			 if(dws.length() !=0) dws = dws.substring(0, dws.length()-1);
	...
	public String doadd() {
		...
		try {
			String hql = " delete RangeSettings ";
			this.cmUserBS.executeUpdate(hql);
			
			if(this.dws2 != null){
			   String[] str = dws2.split(",");
			   for(int i=0;i<str.length;i++){
				   RangeSettings rs = new RangeSettings();
				   rs.setDeptid(str[i].trim().split("-")[1]);
				   rs.setTypeid(str[i].trim().split("-")[2]);
				   this.cmUserBS.save(rs);
			   }
			}
			
			this.message = this.getSuccessMessage("保存成功", "tbuser", "forward", "");
	...
	public String toupdate() {
		try{
			this.companyInfoList = this.cmUserBS.findPageByQuery(" from CompanyInfo where 1=1 ");
			this.cmodelList = this.cmUserBS.findPageByQuery(" from Cmodel where 1=1 ");
                this.rangeSettings = this.cmUserBS.findById(RangeSettings.class,this.cmUser.getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String doupdate() {
		try {			
			...
			this.cmUserBS.update(this.rangeSettings);
			this.message = this.getSuccessMessage("修改成功", 
					null, "forward", "range-settings!toadd.do?pageNum="
	...
	public String dodeleteById() {
		...
		try {
			...
			this.cmUserBS.deleteById(RangeSettings.class,cmUser.getUserId());
	...
	public String multipleDelete() {
		...
		try {
				...
				for (int i = 0; i < condition.length; i++) {
					...
					this.rangeSettings=cmUserBS.findById(RangeSettings.class,condition[i].trim());
	                   this.cmUserBS.delete(this.rangeSettings);
				}
		
				this.message = this.getSuccessMessage(getText("deleteSuccess"), "tbuser", "forward", "range-settings!toadd.do");
	...
	private void getUserList() {
        ...
        String 	hql="from RangeSettings where 1=1 ";
        String counthql="select count(*) from RangeSettings where 1=1  ";

		if(query.get("like_used")!=null && !query.get("like_used").equals("")){
			query.put("like_used", query.get("like_used"));
		}
		sysPageInfo.setPageSize(this.getNumPerPage());
		sysPageInfo.setStartIndex((this.getPageNum()-1)*this.getNumPerPage());
...
this.rangeSettingsList = cmUserBS.findPageByQuery(sysPageInfo, hql, query, sysOrderByInfo);

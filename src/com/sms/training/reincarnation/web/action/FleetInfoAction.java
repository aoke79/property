...
@Results({
	...
	@Result(name="list",location="/sms/training/reincarnation/sfleetinfo.jsp"),
 ...
 * @author LUJIE
 */
public class FleetInfoAction extends BaseAction {
	...
	public String toadd() {
		try{   
			...
			//过滤 不存在的数据
			String hqls = "from FleetInfo where 1=1  order by  id";//degree
			
			List<FleetInfo> ss = this.cmUserBS.findPageByQuery(hqls);
			for(FleetInfo o :  ss){
				
				for(TechnicalGrade c : technicalGradeList){
					
					 if(o.getDegree().equals(c.getDegree())){
						 this.fleetInfoList.add(o);
						 continue;
					 }
				}
		   ...
		   //----
		    List<FleetInfo> ous = new ArrayList<FleetInfo>();
            String info = "";
		    for(FleetInfo o : this.fleetInfoList){
				  ...
				  if(o.getTypeid() != null && o.getDeptid() != null){
					  String hql =" from FleetInfos where 1=1  and typeid='"+o.getTypeid()+"'  and  deptid='"+o.getDeptid()+"'";
					  List<FleetInfos> fs = this.cmUserBS.findPageByQuery(hql);
					  if(fs != null && fs.size() !=0){
						  o.setFjnum(fs.get(0).getPlane());
					  }
		    ...
		    //多条数据进行合并
			List<FleetInfo> ouc = ous;
			
		    for(FleetInfo o : ous){
			    for(FleetInfo o1 : ouc){
			    	//o1.getTypeNameBig()!=null && o.getTypeNameBig()!=null && 
			    	 if(o1.getDeptid()!= null && o.getDeptid() != null &&o1.getTypeid() != null && o.getDegree() != null && o.getTypeid() != null && o1.getDegree() != null ){
			    		...
			    		//  o1.getTypeNameBig().equals(o.getTypeNameBig()) &&
			    		   if(o.getDeptid().equals(o1.getDeptid()) &&  o1.getTypeid().equals(o.getTypeid()) ){
			    			   //if( i!=0 ){
			    				   String[] str = new String[]{o1.getDegree(),o1.getPolits().toString()}; //码表和值
		    ...
		    //计算table所缺少多少列  然后补充
		    String s = "";
		    List<FleetInfo> onew = new ArrayList<FleetInfo>();
		    for(FleetInfo os : ouc){
		    ...
		    //----
		   List<FleetInfo>  oc = new ArrayList<FleetInfo>();
		   for( FleetInfo  opn : onew){
			 //  int  c = 0 ;
		 ...
		 List<Integer[]> xy = new ArrayList<Integer[]>();
		  ...
		  List<FleetInfo> on = new ArrayList<FleetInfo>();
		 ...
		 this.fleetInfoList = on;
		 
		 
		 //计算补加的行数
		 if(this.fleetInfoList != null){
			  this.ands = this.hang-this.fleetInfoList.size();
		 }
	...
	public String doadd() {
		...
		try {
			...
			String hql = " delete FleetInfo ";
			this.cmUserBS.executeUpdate(hql);
			
			String hqlsc = " delete FleetInfos ";
			this.cmUserBS.executeUpdate(hqlsc);
			// 起始页
			int  y = 1;
			if(this.typeid != null){
			...
			for(int i=0;i<typeid.split(",").length;i++){
			    ...
			    for(int cc = 0 ;cc<c ;cc++){
			    	...
			    	if(this.degree.split(",")[start+cc].trim().length() ==0){
			    		...
			    		upg.setPolits(this.degree.split(",")[start+cc].trim());
			    	}
			    	    //获取匹配标识 进行存储
			    	    upg.setDegree(this.technicalGradeList.get(cc).getDegree());
			    	    FleetInfos fs = new FleetInfos();
			    	if(cc ==0){
			    	...
			    	//Cmodel cdd = this.cmUserBS.findById(Cmodel.class, upg.getTypeid());
			    	    if( upg.getTypeid() != null  && upg.getTypeid().length() !=0){
			    	    	...
			    	    	String hqls =" from FleetInfo where 1=1 and typeid='"+upg.getTypeid()+"' and deptid='"+upg.getDeptid()+"' order by degree  ";
			...
			this.message = this.getSuccessMessage("保存成功! (单位或者机型没输入值 那行数据就不能保存.)", "tbuser", "forward", "fleet-info!toadd.do");

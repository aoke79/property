				...
				.bind(
						...
						function(e) {
							var bShowContext = (!!hash[index].onContextMenu) ? hash[index]
									...
									: true;
							if (bShowContext)
								display(index, this, e, options, windowHeight);
							return false
						});
		return this
	};
	function display(index, trigger, e, options, windowHeight) {
		var cur = hash[index];
		...
		if(e.pageY + menu.height() > windowHeight){
			menu.css( {
				'left' : e[cur.eventPosX],
				'top' : e[cur.eventPosY] - menu.height()
			}).show();
		}else{
			menu.css( {
				'left' : e[cur.eventPosX],

...
//定义alt给文本框
$('<label class="alt" for="'+$this.attr("id")+'">'+$this.attr("alt")+'</label>').appendTo($this.parent()).css(altBoxCss(0.5));}  /*调节提示语灰度，1改为了0.5*/
$this.focus(function(){
getAltBox().css(altBoxCss(0.3));}).blur(function(){
if(!$(this).val())getAltBox().show().css("opacity",0.5);}).keydown(function(){  /*调节提示语灰度，1改为了0.5*/
getAltBox().hide();});});},
isTag:function(tn){
if(!tn)return false;
/***chenleilei 添加  如果$(this)[0] 为 undefined 则直接返回  false   在从树中选择节点的时候  alert($(this)[0]+"         $(this)[0]   ");**/
...
/***************************chenleilei  注   这段代码是去掉选择父类第一个子类也被选中********************************************/
//				if (op.ckbox) {
...
//				}
				if (tree.size() > 0) {
					...
					if (closeParentIds.length != 0) {
						$(">div>a", node).each(function(i, n) {
							$(closeParentIds).each(function(j, k) {
										if ($(n).attr("nodeId") == k.toString()) {
											// alert($(n).text());
											op.showSub = false;
										}
									})
					...
					$(">div", node).prepend("<div class='"
							+ (op.showSub ? op.coll : op.exp)  //判断是文件夹前的那个是加号还是减号
							+ "'></div>"
							+ (op.ckbox ? "<div class='ckbox " + checked
									+ "'></div>" : "")
							+ (op.icon ? "<div class='"       //控制那个文件夹是打开还是关闭状态
									+ (op.showSub
					...
					$(">div>div:first,>div>a", node).click(function() {
						...
						(tree.is(":hidden")) ? tree.slideDown("fast") : (isA
								? ""
								: tree.slideUp("fast"));
						return false;
					});
					...
					/************************判断传进来的类型是单位(1)还是部门(2),如果是部门的话则为file样式*********************************/
					var noOrg = $(this);
					var $inputOrg = noOrg.find("a");
					if($inputOrg.attr("ttype")=="2" ){
						$(">div>div:last", node)
										.addClass(op.options.file);	
					}
				} else {
					node.children().wrap("<div></div>");
					/************************判断传进来的类型是单位(1)还是部门(2)，如果是单位的话则样式为folder_expandable，如果是部门的话则为file样式*********************************/
					var noOrg = $(this);
					var $inputOrg = noOrg.find("a");
					if($inputOrg.attr("ttype")=="1"){
						$(">div", node).prepend("<div class='node'></div>"
									+ (op.ckbox ? "<div class='ckbox " + checked
											+ "'></div>" : "")
									+ (op.icon ? "<div class='folder_expandable'></div>" : ""));
					}else{
						$(">div", node).prepend("<div class='node'></div>"
									+ (op.ckbox ? "<div class='ckbox " + checked
											+ "'></div>" : "")
		...
		_check : function(op) {
			...
			/****************chenlei 添加代码 * 当为更新页面时，如果有项被选中，则将此项选中********************************************/
		
			ckbox.append("<input type='checkbox' name='" + $input.attr("tname")
					+ "' value='" + $input.attr("tvalue")
					+ "' style='display:none;'id='"+ $input.attr("tid") +"'/>");			
			ckbox.click(function() {
				...
				/************chenleilei 注   此段代码是将父类树的check框选中且将下面的子类选中**********************************************/
				if(ckbox.parents(".tree").attr("treeType")=="l"){
		...
		_checkParent : function() {
			/**********chenlei  注   这段代码是如果选中子枝叉则将父类枝叉标为绿色******************************/
			if($(this).parents(".tree").attr("treeType")=="l"){
...
$("#switchEnvBox2").switchEnv2();
$("#switchLanguage").switchLanguage();
//$("#switchSysBox").switchEnv();
...
var messageHint = $this.attr("messageHint");
var url=unescape($this.attr("href")).replaceTmById(jParent);
DWZ.debug(url);
if(!url.isFinishedTm()){
alertMsg.error($this.attr("warn")||DWZ.msg("alertSelectMsg"));
return false;}
navTab.openTab(tabid,url,{title:title,messageHint:messageHint,flesh:flesh});
...
var $this=$(this);
$this.accordion({fillSpace:$this.attr("fillSpace"),alwaysOpen:true,active:0});}); /*active控制左边打开第几个面板，默认为0*/
...
//这个alert(message)信息会提示： 有多少个错误信息需要改正
...
if($.fn.removeSelected)$("a[target=removeSelected]",jParent).removeSelected();
...
//add by wangjing start 2011-08-15 在链接dialog之前判断是否选中信息 
if($this.attr("isselect")=='1'){
var cc=$("input[type=checkbox]").filter(function(index) {
           return $(this).attr("checked") && $(this).attr("signflg")=='1';
        });
  if(cc.length<=0){ 
       alertMsg.error($this.attr("warn")||DWZ.msg("alertSelectMsg"));
	   return false;
...
//add by wangjing end 2011-08-15 在链接dialog之前判断是否选中信息
...
var _themeHref=op.themeBase+"/standard/#theme#/style.css";
return this.each(function(){
...
$.fn.switchEnv2=function(){
var op={cities$:">li",boxTitle$:">a>span"};
return this.each(function(){
var $this=$(this);
$this.click(function(){
if($this.hasClass("selected")){
...
$(boxHtml).appendTo("body").css({top:-$(this._boxId).height()+"px"}).animate({top:"200px"},50);
if(this._closeTimer){
clearTimeout(this._closeTimer);
this._closeTimer=null;}

/* 设置当前alertMsg的类型为info或是correct时设置自动关闭的时间(暂时不提供自动关闭，若有需要，可开启此功能) */
//this._types.info==type||
if(this._types.correct==type){
this._closeTimer=setTimeout(function(){alertMsg.close()},5000);
}else{
$(this._bgId).show();}
...
close:function(){
$(this._boxId).animate({top:-$(this._boxId).height()},5,function(){
$(this).remove();});
$(this._bgId).hide();},
...
$(this).find(navTab._op.close$).unbind("click").click(function(){
	$parentTag = $(this).prev().parent();
	$leaveTips = $parentTag.find(":first").attr("messageHint");
	if($leaveTips != "undefined"){
		alertMsg.confirm($leaveTips,{okCall:function(){
...
//主页刷新-----Begin
if($tab.attr("tabid")=="main"){
//$tab.data("url","standard!index.do");
$tab.data("tabid","main");
//if(flag){
...
if(url.indexOf("pageNum")<0&&url.indexOf("numPerPage")<0&&url.indexOf("strQuery")<0&&url.indexOf("orderBlock")<0&&url.indexOf("orderMethod")<0&&url.indexOf("parentId")<0&&url.indexOf("currentTreeNode")<0){
	param = {
	"pageNum":pagination,
	"numPerPage":pageSize,
	"strQuery":query,
	"orderBlock":orderBy,
	"orderMethod":orderCol,
	"parentId":parentId,
	"currentTreeNode":currentTreeNode
};
}
if($panel)$panel.loadUrl(url,param,function(){
...
var tabFrag='<li tabid="#tabid#"><a href="javascript:" messageHint="#messageHint#" title="#title#"><span>#title#</span></a><a href="javascript:void(0)" class="close">close</a></li>';
this._tabBox.append(tabFrag.replace("#tabid#",tabid).replaceAll("#title#",op.title).replace("#messageHint#",op.messageHint));
...
if($orderField){
	$th.addClass(style[1]).click(function(){
		
		var $obj = $(this).parents("div .pageContent").prev().children('form');
		//使用计数器来判断当前排序的状态
		if(iCount%2 == 0){
			$orderType = 1;
			$th.removeClass("asc");
		...
		$obj.children('#orderMethod').attr("value",$orderType);
		$obj.children('#orderField').attr("value",$orderField);
		$obj.children('#fieldType').attr("value", $fieldType);
		
		$obj.children('#isOrder').attr("value","yes");
		return navTabSearch($obj);
	}).removeAttr("align").hoverClass("hover").removeAttr("width").width(style[0]);
...
if($ftd.attr("sceneFlag")=="yes"){ 
  $ftd.mouseover(function(){
     var strDay=$(this).attr("strDay");
     //给三个td分别加背景颜色
     $("td[strDay="+strDay+"]").css("background","LightGrey").css("border-color","LightGrey"); 
   }).click(function(){
       var strDay=$(this).attr("strDay");
       $("#"+strDay).click();//触发链接时间。跳转到排课增加或者修改页面
   });
  $ftd.mouseout(function(){
    var strDay=$(this).attr("strDay"); 
    $("td[strDay="+strDay+"]").css("background","").css("border-color",""); 
  }); 
}

if(typeof($ftd.attr("clip")) != "undefined"){
	$ftd.html("<div style='height:30px'>"+$ftd.html()+"</div>");
...
$tr.click(function(){
if($tr.attr("noCss")!="true"){//排课页面点击一个tr不显示背景色 
  $trs.filter(".selected").removeClass("selected"); 
  $tr.addClass("selected");
  var sTarget=$tr.attr("target");
  if(sTarget){
...
navTab.reload($(form).attr("action"),$(form).serializeArray(),navTabId);
return false;}
...
if(args["fieldType"])form.fieldType.value=args["fieldType"];
}
return form;}
...
var maxPages = pc.numPages();
//判断当输入的页码大于最大页数时，自动跳至最后一页
if ((pageNum) > maxPages){
	//alert(pageNum);
	pageNum = maxPages+"";
}
...
var maxPages=this.numPages();
if(isNaN(currentPage) || currentPage > maxPages)return maxPages;
return currentPage;},
hasPrev:function(){
...
// ========= add by jilili  控制时间的开始时间与结束时间  start===============//
var year = $(setting.year$).val();
if(minDay != undefined){
$days.each(function(){
	var month = parseInt($(setting.month$).val());
	var day = $(this).attr("day");
	var date = new Date();
	var chMonth = $(this).attr("chMonth");
	if(chMonth!= undefined){
		month += parseInt(chMonth);
	}
	date.setFullYear(year, month-1, day);
	var dwDay = date.getTime();
	var dayDate = new Date();
	dayDate.setHours(0, 0, 0, 0);
	dayDate = dayDate.getTime();
	dayDate = dayDate - (minDay)*24*60*60*1000;
	var correctDay = new Date();
	correctDay.setTime(dayDate);
	$(setting.year$).children("option").each(function(){
		if($(this).val() < correctDay.getFullYear() - 1 ){
			$(this).remove();
		}
	});
	
	$(setting.month$).children("option").each(function(){
		if($(this).val() < correctDay.getMonth()){
			$(this).remove();
		}
	});
...
if(maxDay != undefined){
	$days.each(function(){
		...
		date.setFullYear(year, month-1, day);
		var dwDay = date.getTime();
		var dayDate = new Date().getTime();
		dayDate = dayDate + (maxDay)*24*60*60*1000;
		var correctDay = new Date();
		correctDay.setTime(dayDate);
		$(setting.year$).children("option").each(function(){
			if($(this).val() > correctDay.getFullYear()){
				$(this).remove();
			}
		});
		
		$(setting.month$).children("option").each(function(){
			if($(this).val() > correctDay.getMonth() + 1){
				$(this).remove();
			}
		});
...
// ========= add by jilili  控制时间的开始时间与结束时间  end===============//
$days.click(function(){
	...
	/**
	 * 根据cape属性的值来判断是否触发以下事件
	 * 此事件需要指定三个属性：
	 *     属性cape为timeLimited;
	 *     属性limit为限制数值,例：limit="3";
	 *     属性tips为当前绑定的input对应的名称,一般此类名称通常包含期限
	 */
	if($this.attr("cape")){
		//当前时间
		var cdateNow = new Date();
		var cyear = cdateNow.getFullYear();
		var cmonth = cdateNow.getMonth() + 1;
		var cday = cdateNow.getDate();
		var cDate = cyear + "-" + cmonth + "-" + cday;
		//比较选出的时间和当前时间，并执行alertMsg中的部分
		var selectedDate = new Date($this.val().replace(/-/g,"/"));
		var currentDate = new Date(cDate.replace(/-/g,"/"));
		var diffDate = parseInt((selectedDate - currentDate) / (1000*60*60*24));
		
		var info = "";
		if($this.attr("cape").indexOf("timeLimited") != -1){
			var limitedVal = $this.attr("limit");
			if(diffDate < limitedVal){
				if(diffDate > 0){
					info = '最低'+$this.attr("tips")+'为'+limitedVal+'天！';
				} else {
					info = $this.attr("tips")+'不能小于当前日期！';
				}
				alertMsg.info(info);
				$this.val("");
			}
		}
		if($this.attr("cape").indexOf("noBefore") != -1) {
			if(diffDate < 0) {
				info = $this.attr("tips")+'不能小于当前日期！';
				alertMsg.info(info);
				$this.val("");
		...
		jPanel : function(options) {
			...
			return this.each(function() {
						...
						var footer = $("<div><div></div></div>")
								.appendTo($panel).addClass(op.footer);
...
if($this.attr("noAjax")=="true"){
 options+="<li><a style=\"text-decoration: none;\"  class=\""+ (value==option[0].value?"selected":"") +"\" href=\"javascript:deal('"+id+"');\" value=\"" + option[0].value + "\">"+option[0].text+"</a></li>";
}else if($this.attr("noAjax")=="noDeal"){
  options+="<li><a style=\"text-decoration: none;\"  class=\""+ (value==option[0].value?"selected":"") +"\" href=\"javascript:deal('"+option[0].value+"');\" value=\"" + option[0].value + "\">"+option[0].text+"</a></li>";
}else if($this.attr("noAjax")=="train"){
  options+="<li><a style=\"text-decoration: none;\"  class=\""+ (value==option[0].value?"selected":"") +"\" href=\"javascript:void(0);\" value=\"" + option[0].value + "\">"+option[0].text+"</a></li>";
}else{

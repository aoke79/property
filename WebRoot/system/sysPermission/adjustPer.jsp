...
$(function(){ 
	  var OPIDs=$("#hiddenOPIDs").val();
	  //初始化树结构，把属于当前机构的操作权限选中
   	    $("a[flag=sinoframe]").filter(function(index) {
         return OPIDs.indexOf($(this).attr("tid"))!=-1;
        }).attr("checked","true");
	   // 单击清空功能的时候
	  ...
	  // 当单击取消按钮的时候
	   ...
	   $("button[name=subUser]").click(function(){
			...
			$("button.close").click();
			$("#pagerForm").attr("action","sys-permission/sys-permission!doAdjust.do?OPIDs=" + ids);
...
<s:property value="baseOperate.subOperate"/>
<s:property value="#application.initBaseOperate.subOperate"/>--%>

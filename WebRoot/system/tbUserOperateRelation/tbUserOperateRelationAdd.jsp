...
$(function(){ 
	  var UOIDs=$("#hiddenUOIDs").val();
	  //初始化树结构，把属于当前用户的操作权限选中
   	    $("a[flag=sinoframe]").filter(function(index) {
         return UOIDs.indexOf($(this).attr("tid"))!=-1;
        }).attr("checked","true");
	   // 单击清空功能的时候
	  ...
	  // 当单击取消按钮的时候
	   ...
	   $("button[name=subUser]").click(function(){
			...
			$("button.close").click();
			$("#pagerForm").attr("action","cm-user-operate-relation/cm-user-operate-relation!doFp.do?UOIDs=" + ids + "&userId=" + $("input[name=userId]").val());//UOIDs -> operateIds
...
<input type="hidden" id="userId" name="userId" value= "${cmUser.userId}"/> 

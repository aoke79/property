...
<script>
	$('a[flag=menuTrees]').contextMenu('menus', {
      //菜单样式
      menuStyle: {
      ...
      bindings: 
          {
            'menuAdd': function(t) {
            	var url = "sys-menu/sys-menu!add.do?sysMenu.subsystemId="+$(t).attr("sysid")+"&sysMenuName=" + $(t).attr("nodeId");
...
<div class="contextMenu" id="menus">
      <ul>
        <li id="menuAdd"><img src="<%=basePath %>/images/smart_add.gif"/>&nbsp;<a class="menuItem" href="#"  messageHint="<s:property value='getText("addLeaveTips")' />" target="navTab"><s:property value="getText('add')"/></a></li>
      </ul>
</div>
...
<s:if test="#attr.users=='超级管理员'">
		<s:iterator value="systemList" var="first">
				<li><a flag="menuTrees" nodeId="-1" sysid="${id}"><s:if test="%{#session['WW_TRANS_I18N_LOCALE'].equals(@java.util.Locale@US)}">${enName}</s:if><s:else>${name}</s:else></a>
				<s:iterator value="menuList.{?#this.subsystemId==#first.id && #this.parentMenu.id=='-1'}" var ="second">
					<ul>
						<li><a flag="menuTree" nodeId="${id}" sysid="${subsystemId}"><s:if test="%{#session['WW_TRANS_I18N_LOCALE'].equals(@java.util.Locale@US)}">${enName}</s:if><s:else>${name}</s:else></a>
														...
														<ul>
															<li><a flag="menuTree" nodeId="${id}" sysid="${subsystemId}"><s:if test="%{#session['WW_TRANS_I18N_LOCALE'].equals(@java.util.Locale@US)}">${enName}</s:if><s:else>${name}</s:else></a>
																<s:iterator value="menuList.{?#this.parentMenu != null && #this.parentMenu.id==#fifth.id}">
																	<ul>
																		<li><a flag="menuTree" nodeId="${id}" sysid="${subsystemId}"><s:if test="%{#session['WW_TRANS_I18N_LOCALE'].equals(@java.util.Locale@US)}">${enName}</s:if><s:else>${name}</s:else></a></li>
																	</ul>
																</s:iterator>
															</li>
														</ul>
...
<s:if test="#attr.users!='超级管理员'">
		<s:iterator value="systemList" var="first">
				<li><a flag="menuTrees" nodeId="${id}"><s:if test="%{#session['WW_TRANS_I18N_LOCALE'].equals(@java.util.Locale@US)}">${enName}</s:if><s:else>${name}</s:else></a>
